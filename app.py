...
function renderChain(){
 ...
 if(best){const x=best.x,r=best.r;window._bestContract=x;bestOptionBox.innerHTML='<div class="label">FACT-BASED CONTRACT HIGHLIGHT</div><div class="value" style="font-size:20px">'+esc(C.ticker)+' '+esc(x.expiration)+' '+money(x.strike)+' '+esc(x.type.toUpperCase())+' <span class="besttag">TOP SCREEN '+r.score+'/100</span></div><div class="sub">Ranks contract mechanics only: liquidity, spread, modeled decay, IV versus realized volatility, breakeven distance and time to expiration. It does not decide market direction or tell you to buy. '+esc(r.parts.join(' · '))+'</div><button class="btn" style="margin-top:9px" onclick="selectBestContract()">Analyze highlighted contract</button>';}else bestOptionBox.innerHTML='<div class="label">FACT-BASED CONTRACT HIGHLIGHT</div><div class="sub">No contract currently passes the screen.</div>';
