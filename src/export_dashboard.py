...
def analyze(ticker):
    ...
    options={"status":"Historical distribution + delayed option chain active","horizons":horizons,"realized_vol_20d":sf(daily.rolling(20).std().iloc[-1]*sqrt(252))}\n    try: options["chain"]=option_snapshot(ticker,float(close.iloc[-1]))\n    except Exception as e: options["chain"]={"error":str(e),"contracts":[],"quote_note":"Option chain unavailable for this run."}
